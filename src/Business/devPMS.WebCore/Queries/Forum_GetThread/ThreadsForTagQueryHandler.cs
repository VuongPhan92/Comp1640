...
{
    public class ThreadsForTagQueryHandler : IQueryHandler<ThreadsForTagQuery, PagedListResult<Thread>>
    {
        private readonly IDbContextFactory _factory;

        public ThreadsForTagQueryHandler(IDbContextFactory factory)
        {
            _factory = factory;
        }

        public PagedListResult<Thread> Handle(ThreadsForTagQuery query)
        {
            using (var uow = _factory.Create(DBContextName.UniversityEntities))
            {
                ...
                searchQuery.AddFilter(p => p.IsPublished == true && p.Tags.Any(t => t.UrlSeo.Equals(query.UrlSeo)));
